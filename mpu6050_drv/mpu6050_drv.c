 ...
 *返回值，错误，-1。成功，0
 */
static int i2c_write_mpu6050(struct i2c_client *mpu6050_client, u8 address, u8 *data, u32 length)
{
	int error = 0;
	u8 write_data[length + 1];
...
static int mpu6050_init(void)
{
	int error = 0;
	/*配置mpu6050*/
	char init_data[4] = {0x00,0x07,0x06,0x01};
	error += i2c_write_mpu6050(mpu6050_client, PWR_MGMT_1, &init_data[0], 1);
	error += i2c_write_mpu6050(mpu6050_client, SMPLRT_DIV, &init_data[1], 1);
	error += i2c_write_mpu6050(mpu6050_client, CONFIG, &init_data[2], 1);
	error += i2c_write_mpu6050(mpu6050_client, ACCEL_CONFIG, &init_data[3], 1);
...
static ssize_t mpu6050_read(struct file *filp, char __user *buf, size_t cnt, loff_t *off)
{
	int error;
	u8 reg_addr = buf[0];
	u8 *data_buffer = &buf[1];
	u32 length = cnt - 1;

	i2c_read_mpu6050(mpu6050_client, reg_addr, data_buffer, length);

	/*将读取得到的数据拷贝到用户空间*/
	error = copy_to_user(buf, data_buffer, cnt - 1);
...
static ssize_t mpu6050_write(struct file *filp, const char __user *buf, size_t cnt, loff_t *ppos)
{
	int error;
	u8 reg_addr = buf[0];
	const u8 *data_buffer = &buf[1];
	u32 length = cnt - 1;
	int tmp = cnt;

	error = copy_from_user(data_buffer, buf, cnt - 1);
	if (error != 0)
	{
		printk("copy_to_user error!");
		return -1;
	...
	*ppos += tmp;
	...
	i2c_write_mpu6050(mpu6050_client, reg_addr, data_buffer, length);
